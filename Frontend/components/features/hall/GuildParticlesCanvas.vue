...
function render() {
  ...
  for (let i = 0; i < particles.length; i++) {
    ...
    if (p.life < p.maxLife * 0.15) {
      ...
      p.alpha = (1 - (p.life - p.maxLife * 0.75) / (p.maxLife * 0.25)) * p.maxAlpha
    } else {
      p.alpha = p.maxAlpha
    }
    ...
    ctx.shadowBlur = p.type === 'ember' ? 12 : 6
    ...
    if (p.life >= p.maxLife || p.y < 0 || p.x < 0 || p.x > width) {
      particles[i] = p.type === 'ember' ? createEmber(width, height) : createDust(width, height)
    }
  }
