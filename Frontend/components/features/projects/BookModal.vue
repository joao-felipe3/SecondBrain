<template>
  <v-row
    ...
    :class="['book-modal-overlay', { 'is-closing': isClosing }]"
    @click="onBackdrop"
  >
    <v-container class="container" @click.stop>
      <button
        class="icon-floating-close"
        @click="closeModal"
        ...
        <v-col class="d-flex justify-center">
          <v-sheet class="book" elevation="2" color="transparent">
            ...
            <div
              ref="carouselEl"
              class="carousel"
              :style="{ '--slides': `${TOTAL_SLIDES}` }"
            >
              <v-sheet
                :class="['carousel-item', { active: currentIndex === 0 }]"
                elevation="0"
                color="transparent"
                ><GeneralInfoPage
              ...
              /></v-sheet>
              <v-sheet
                :class="['carousel-item', { active: currentIndex === 1 }]"
                elevation="0"
                color="transparent"
                ><SmartObjectivesPage
                  :project="renderProject || {}"
                  :editing="editing"
                  @update-field="updateField"
                  @smart-objective-updated="reloadProject"
              /></v-sheet>
              <v-sheet
                :class="['carousel-item', { active: currentIndex === 2 }]"
                elevation="0"
                color="transparent"
                ><WBSPage
                  :project="renderProject || {}"
                  :editing="editing"
                  @wbs-updated="reloadProject"
              /></v-sheet>
              <v-sheet
                :class="['carousel-item', { active: currentIndex === 3 }]"
                elevation="0"
                color="transparent"
                ><CriticalPathPage
                  :project="renderProject || {}"
              ...
              /></v-sheet>
              <v-sheet
                :class="['carousel-item', { active: currentIndex === 4 }]"
                elevation="0"
                color="transparent"
                ><RTMPage
                  :project="renderProject as Project"
                  :editing="editing"
              /></v-sheet>
              <v-sheet
                :class="['carousel-item', { active: currentIndex === 5 }]"
                elevation="0"
                color="transparent"
                ><RiskPage
                  :project="renderProject as Project"
              ...
              /></v-sheet>
              <v-sheet
                :class="['carousel-item', { active: currentIndex === 6 }]"
                elevation="0"
                color="transparent"
                ><GanttPage
                  :project="renderProject as Project"
              ...
              /></v-sheet>
              <v-sheet
                :class="['carousel-item', { active: currentIndex === 7 }]"
                elevation="0"
                color="transparent"
                ><PertDiagramPage
                  :project="renderProject as Project"
              ...
              /></v-sheet>
              <v-sheet
                :class="['carousel-item', { active: currentIndex === 8 }]"
                elevation="0"
                color="transparent"
                ><XMatrixPage
                  :project="renderProject as Project"
              ...
              /></v-sheet>
              <v-sheet
                :class="['carousel-item', { active: currentIndex === 9 }]"
                elevation="0"
                color="transparent"
                ><BacklogAndProgress
                  :project="renderProject || {}"
...
import {
  useProjectEditing,
  getProjectId,
} from "~/composables/features/useProjectEditing";
...
const renderProject = computed(() => {
  ...
  const hasSmartObjective = Boolean(
    ...
    (smartObjective.summary?.trim() ||
      ...
      (Array.isArray(smartObjective.risks) &&
        smartObjective.risks.length > 0) ||
    ...
    if (newProject?._id) {
      try {
        const response = await fetch(
          `http://localhost:3000/projects/${newProject._id}/tasks`,
        );
