...
export const useTaskStore = defineStore("task", {
  ...
  getters: {
    activeTasks: (state) =>
      state.tasks.filter((t) => !isRecurringTemplate(t) && !t.isConcluded),
    ...
    lateTasks: (state) =>
      state.tasks.filter(
        (t) => !isRecurringTemplate(t) && t.late && !t.isConcluded,
      ),
    ...
    tasksByProject: (state) => (projectId: string) =>
      state.tasks.filter(
        (t) => !isRecurringTemplate(t) && t.project === projectId,
      ),
    ...
    getTasksByStatus:
      (state) =>
      (status: "todo" | "doing" | "review" | "done", projectId?: string) => {
        let filtered = state.tasks.filter(
          (t) => !isRecurringTemplate(t) && t.status === status,
        );
    ...
    habitDashboardHabits: (state) => state.habitsDashboard?.habits || [],
    ...
    getHabits: (state) => (filters?: { projectId?: string }) => {
      let list = state.tasks.filter(
        (t) =>
          !isRecurringTemplate(t) &&
          (t.microTaskType === "habit" ||
            Boolean(t.recurringRule) ||
            Boolean(t.parentRecurringId)),
      );
    ...
    getStreakForHabit: (state) => (parentRecurringId: string) => {
      if (
        !state.habitsDashboard ||
        !Array.isArray(state.habitsDashboard.habits)
      ) {
        return { currentStreak: 0, longestStreak: 0, aderencePercent: 0 };
      }
      const found = state.habitsDashboard.habits.find(
        (h) => String(h.id) === String(parentRecurringId),
      );
      if (!found)
        return { currentStreak: 0, longestStreak: 0, aderencePercent: 0 };
      return {
    ...
    getHabitsByStreak: (state) => () => {
      const list = state.habitsDashboard?.habits
        ? state.habitsDashboard.habits.slice()
        : [];
      return list.sort(
  ...
  actions: {
    ...
    async loadHabitsDashboard(projectId?: string) {
      ...
      try {
        const tasksApi = useTasksApi();
        const { data, error } = await tasksApi.fetchHabitsDashboard(projectId);
    ...
    async createTask(newTask: Partial<Task>) {
      ...
      const payload: any = cleanPayload({
        ...newTask,
        pomodorosPlanned:
          newTask.pomodorosPlanned === null ||
          newTask.pomodorosPlanned === undefined
            ? 1
    ...
    async createHabit(newTask: Partial<Task>) {
      ...
      const cleaned: any = cleanPayload({
        ...payload,
        pomodorosPlanned:
          payload.pomodorosPlanned === null ||
          payload.pomodorosPlanned === undefined
            ? 1
     ...
     */
    async updateMicroTaskChecklistItem(
      taskId: string,
      itemIndex: number,
      completed: boolean,
    ) {
     ...
     */
    async updateMicroTaskChecklistFull(
      id: string,
    ...
    async setTaskStatus(
      id: string,
      toStatus: "todo" | "doing" | "review" | "done",
    ) {
      try {
      ...
      if (!error && data) {
        ...
        if (this.habitsDashboard?.habits) {
          ...
          if (dashboardHabit) {
            ...
            dashboardHabit.currentStreak = Math.max(
              0,
    ...
    async handleRecurringCompletion(id: string) {
      ...
      if (!error && nextOccurrence) {
        ...
        if (this.habitsDashboard?.habits) {
          const dashboardHabit = this.habitsDashboard.habits.find(
            (habit) =>
              habit.id === id || habit.id === nextOccurrence.parentRecurringId,
          );
          if (dashboardHabit && nextOccurrence.streakData) {
            dashboardHabit.currentStreak =
              nextOccurrence.streakData.currentStreak || 0;
            dashboardHabit.aderencePercent =
              nextOccurrence.streakData.aderencePercent || 0;
          }
        }

        return {
          success: true,
          data: { completed: moveResult.data, nextOccurrence },
        };
      }

      // Se não conseguiu gerar próxima, ainda assim a conclusão foi bem-sucedida
      console.warn(
        "Não foi possível gerar próxima ocorrência:",
        error?.message,
      );
    ...
    async deleteRecurringSeries(parentRecurringId: string) {
      ...
      if (!error) {
        this.tasks = this.tasks.filter(
          (task) =>
            task._id !== parentRecurringId &&
            task.parentRecurringId !== parentRecurringId,
        );

        if (this.habitsDashboard?.habits) {
          this.habitsDashboard.habits = this.habitsDashboard.habits.filter(
            (habit) => habit.id !== parentRecurringId,
          );
        }
