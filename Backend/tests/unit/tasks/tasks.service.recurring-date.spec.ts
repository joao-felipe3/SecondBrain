...
interface PrivateTasksRecurringService {
...
describe('TasksService - calculateNextRecurringDate Logic', () => {
  ...
  let helper: PrivateTasksRecurringService;

  beforeEach(() => {
    service = new TasksRecurringService();
    helper = service as unknown as PrivateTasksRecurringService;
  });
  ...
  it('should handle weekly with multiple daysOfWeek correctly', () => {
    ...
    const result = helper.calculateNextRecurringDate(tuesday, {
      frequency: 'weekly',
      interval: 1,
      daysOfWeek: [3, 5],
    });
  ...
  it('should wrap to next week if all daysOfWeek are passed', () => {
    ...
    const result = helper.calculateNextRecurringDate(friday, {
      frequency: 'weekly',
      interval: 1,
      daysOfWeek: [1, 3],
    });
  ...
  it('should handle daily with interval > 1', () => {
    const result = helper.calculateNextRecurringDate(new Date('2026-04-20T10:00:00.000Z'), {
      frequency: 'daily',
      interval: 5,
    });
  ...
  it('should handle monthly on 31st (edge month boundaries)', () => {
    ...
    const result = helper.calculateNextRecurringDate(may31, {
      frequency: 'monthly',
      interval: 1,
    });
  ...
  it('should respect endDate boundary correctly', () => {
    ...
    const result = helper.calculateNextRecurringDate(new Date('2026-04-25T10:00:00.000Z'), {
      frequency: 'daily',
      interval: 1,
