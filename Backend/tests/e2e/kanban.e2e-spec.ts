...
interface LineageResponse {
  ancestors?: Array<{ parentTaskId?: string | null }>;
  children?: unknown[];
  warnings?: unknown[];
}
...
describe('Sprint 4: Kanban Board E2E Tests', () => {
  ...
  describe('Task Status Persistence', () => {
    ...
    it('should move task from todo to doing and persist', async () => {
      ...
      // Move to 'doing'
      const moveResponse = await request(app.getHttpServer() as App)
        .patch(`/tasks/${taskId}/status`)
        .send({ status: 'doing' })
        .expect(200);

      const moveBody = moveResponse.body as TaskResponse;
      expect(moveBody.status).toBe('doing');
      ...
      const refreshBody = refreshResponse.body as TaskResponse;
      expect(refreshBody.status).toBe('doing');
      expect(refreshBody.statusUpdatedAt).toBeDefined();
    });
    ...
    it('should allow status transitions: todo -> review -> done', async () => {
      ...
      // todo -> review
      const reviewResponse = await request(app.getHttpServer() as App)
        .patch(`/tasks/${taskId}/status`)
        .send({ status: 'review' })
        .expect(200);

      const reviewBody = reviewResponse.body as TaskResponse;
      expect(reviewBody.status).toBe('review');

      // review -> done (should trigger completion flow)
      const doneResponse = await request(app.getHttpServer() as App)
        .patch(`/tasks/${taskId}/status`)
        .send({ status: 'done' })
        .expect(200);

      const doneBody = doneResponse.body as TaskResponse;
      expect(doneBody.status).toBe('done');
      expect(doneBody.isConcluded).toBe(true);
    });
  ...
  describe('Kanban Board Ordering', () => {
    it('should append task to end of destination column', async () => {
      ...
      // Create 3 tasks
      const task1 = await request(app.getHttpServer() as App)
        .post('/tasks')
        .send({ name: 'Task 1', project: projectId })
        .expect(201);

      const task2 = await request(app.getHttpServer() as App)
        .post('/tasks')
        .send({ name: 'Task 2', project: projectId })
        .expect(201);

      const task3 = await request(app.getHttpServer() as App)
        .post('/tasks')
        .send({ name: 'Task 3', project: projectId })
        .expect(201);

      const task1Body = task1.body as TaskResponse;
      const task2Body = task2.body as TaskResponse;
      const task3Body = task3.body as TaskResponse;

      // Move all to 'doing'
      await request(app.getHttpServer() as App)
        .patch(`/tasks/${task1Body._id || ''}/status`)
        .send({ status: 'doing' })
        .expect(200);

      await request(app.getHttpServer() as App)
        .patch(`/tasks/${task2Body._id || ''}/status`)
        .send({ status: 'doing' })
        .expect(200);

      await request(app.getHttpServer() as App)
        .patch(`/tasks/${task3Body._id || ''}/status`)
        .send({ status: 'doing' })
        .expect(200);
      ...
      expect(bodyList.length).toBe(3);
      expect(bodyList[0]?.kanbanOrder ?? 0).toBeLessThan(bodyList[1]?.kanbanOrder ?? 0);
      expect(bodyList[1]?.kanbanOrder ?? 0).toBeLessThan(bodyList[2]?.kanbanOrder ?? 0);
    });
  });
  ...
  describe('Lineage Tracking', () => {
    it('should return task genealogy (ancestors + children)', async () => {
      ...
      const body = response.body as LineageResponse;
    ...
    it('should show parent chain for nested tasks', async () => {
      ...
      expect(ancestors[ancestors.length - 1]?.parentTaskId).toBeNull();
    });
  });
  ...
  describe('Completion Feedback', () => {
    it('should generate feedback when task moved to done', async () => {
      ...
      const body = response.body as FeedbackResponse;
    ...
    it('should return latest feedback for task', async () => {
      ...
      const secondBody = second.body as FeedbackResponse;
      ...
      const responseBody = response.body as FeedbackResponse;
      // Should return the most recent one (by createdAt)
      expect(responseBody.feedback).toBe(secondBody.feedback);
    });
    ...
    it('should persist user-submitted completion feedback after concluding a task', async () => {
      ...
      const createBody = createRes.body as TaskResponse;
      ...
      const postResponse = await request(app.getHttpServer() as App)
        .post(`/tasks/${taskId}/completion-feedback`)
        .send(userFeedbackPayload)
        .expect(200);

      const postBody = postResponse.body as FeedbackResponse;
      expect(postBody.feedback).toContain('Entrega concluída com sucesso');

      const getResponse = await request(app.getHttpServer() as App)
        .get(`/tasks/${taskId}/completion-feedback`)
        .expect(200);

      const getBody = getResponse.body as FeedbackResponse;
      expect(getBody.feedback).toContain('Entrega concluída com sucesso');
      expect(getBody.feedback).toContain('Nenhum impeditivo relevante');
    });
  });
  ...
  describe('Kanban Board Constraints', () => {
    it('should block moving to done if checklist incomplete', async () => {
      ...
      const body = response.body as TaskResponse;
      expect(body.message).toContain('Checklist');
    });
  ...
  describe('Sprint 4 Full Scenario', () => {
    it('should complete full Kanban workflow', async () => {
      ...
      const createBody = createRes.body as TaskResponse;
      const taskId = createBody._id || '';
      expect(createBody.status).toBe('todo');

      // 2. Move to doing
      const doingRes = await request(app.getHttpServer() as App)
        .patch(`/tasks/${taskId}/status`)
        .send({ status: 'doing' })
        .expect(200);

      const doingBody = doingRes.body as TaskResponse;
      expect(doingBody.status).toBe('doing');

      // 3. Check lineage
      const lineageRes = await request(app.getHttpServer() as App)
        .get(`/tasks/${taskId}/lineage`)
        .expect(200);

      const lineageBody = lineageRes.body as LineageResponse;
      expect(lineageBody).toHaveProperty('ancestors');
      expect(lineageBody).toHaveProperty('children');

      // 4. Move to review
      const reviewRes = await request(app.getHttpServer() as App)
        .patch(`/tasks/${taskId}/status`)
        .send({ status: 'review' })
        .expect(200);

      const reviewBody = reviewRes.body as TaskResponse;
      expect(reviewBody.status).toBe('review');
      ...
      // 6. Generate feedback
      const feedbackRes = await request(app.getHttpServer() as App)
        .post(`/tasks/${taskId}/completion-feedback`)
        .expect(200);

      const feedbackBody = feedbackRes.body as FeedbackResponse;
      expect(feedbackBody).toHaveProperty('feedback');
      ...
      const refreshBody = refreshRes.body as TaskResponse;
      expect(refreshBody.status).toBe('done');
    });
  });
});
