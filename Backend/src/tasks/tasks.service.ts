...
export class TasksService {
  ...
  async createMany(createTaskDtos: CreateTaskDto[], options?: CreateManyTasksOptionsDto): Promise<TaskDocument[]> {
  ...
  async createRecurringTemplate(createMicroTaskDto: CreateMicroTaskDto): Promise<TaskDocument> {
    const recurringRule = this.tasksRecurringService.normalizeRecurringRule(
      createMicroTaskDto.recurringRule as RecurringRuleDto | undefined
    );
  ...
  async createRecurringMicroTask(createMicroTaskDto: CreateMicroTaskDto): Promise<TaskDocument> {
    ...
    const firstDeadline = this.tasksRecurringService.calculateFirstRecurringDate(referenceStart, recurringRule);
    if (!firstDeadline) {
      return template;
    }
  ...
  async remove(id: string): Promise<boolean> {
    ...
    async generateChecklistViaCopilotWithHistory(
      ...
      return this.tasksChecklistService.generateChecklistWithHistory(taskName, description, microTaskType, projectId);
    }

    async updateChecklistItem(taskId: string, itemIndex: string, completed: boolean): Promise<TaskDocument> {
    ...
    async updateRecurringRule(id: string, recurringRule: RecurringRuleDto): Promise<TaskDocument> {
      ...
      const updatedTask = await this.taskModel
        .findByIdAndUpdate(id, { recurringRule: this.tasksRecurringService.normalizeRecurringRule(recurringRule) }, { new: true })
        .exec();
      if (!updatedTask) {
        throw new NotFoundException(`Task with id ${id} not found`);
      }
    ...
    async generateNextOccurrence(taskOrId: string | TaskDocument): Promise<TaskDocument | null> {
      ...
      const recurringRule = task.recurringRule ? this.tasksRecurringService.normalizeRecurringRule(task.recurringRule) : undefined;
      if (!recurringRule) {
        return null;
      }

      const nextDeadline = this.tasksRecurringService.calculateNextRecurringDate(task.deadline || task.createdAt || new Date(), recurringRule);
      if (!nextDeadline) {
        return null;
      }

      return this.createTaskCore(this.tasksRecurringService.buildOccurrencePayload(task, nextDeadline) as any);
    }
    ...
    async suggestPertEstimates(taskType: string, description: string, projectContext?: string): Promise<any> {
