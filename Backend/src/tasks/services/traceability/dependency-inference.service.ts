...
export class DependencyInferenceService {
    ...
    private dependencyTupleSchema = z.tuple([
        z.string().min(1),
        z.string().min(1),
    ...
    async inferWithAi(params: {
        ...
        const tryOnce = async (p: string, edges: number, tokens: number) => {
            ...
            const raw = Array.isArray((validated as any)?.dependencies)
                ? ((validated as any).dependencies as any[])
                : [];
            ...
            if (this.isVerbose()) {
                this.logger.log(
                    `[dep-infer] done requestId=${requestId || '-'} raw=${raw.length} normalized=${normalizedCount} ` +
                        `filtered=${filteredCount} sliced=${slicedCount} acyclic=${acyclicCount} durationMs=${Date.now() - startedAt}`,
                );
            }
